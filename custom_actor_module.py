...
img_crop_top = cfg.IMG_CROP_TOP
img_crop_bottom = cfg.IMG_CROP_BOTTOM
...
class VanillaCNN(nn.Module):
    def __init__(self, q_net):
        ...
        # The vertical height is scaled to img_height, then trimmed manually to 64 pixels
        self.h_out, self.w_out = 64, img_width
    ...
    def forward(self, x):
        ...
        images = images[:, :, 48:112, :]  # I might have the direction of the y axis backwards
...
class MyActorModule(TorchActorModule):
    ...
    def save(self, path):
        ...
        IN PARTICULAR, WE *DO NOT* ACCEPT PICKfLE FILES (such as output by torch.save()...).
...
if __name__ == "__main__":
    ...
    if args.trainer:
        my_trainer = Trainer(training_cls=training_cls,
                             ...
                             password=password,
                             security=security)
        # my_trainer.run_with_wandb(entity=wandb_entity, project=wandb_project, run_id=wandb_run_id)
        my_trainer.run()
